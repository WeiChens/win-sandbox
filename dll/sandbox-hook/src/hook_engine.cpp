...
// NtQueryObject 类型（★ 替代 GetFinalPathNameByHandleW 的轻量方案）
...
// ============================================================================
// ★ NtQueryObject 路径解析（轻量级，无递归风险）
//
// Trae 方案：比 GetFinalPathNameByHandleW 快 100 倍
// - 直接在对象管理器层查询，不经过文件系统驱动栈
// - 不触发任何文件 I/O → 不会递归触发 Hook
// - 无需 TLS 重入守卫
// ============================================================================

// NtQueryObject 函数指针（延迟解析）
...
static bool GetPathFromHandleNt(HANDLE hFile, wchar_t* out, size_t outSize) {
    ...
    // 预期返回 STATUS_INFO_LENGTH_MISMATCH (0xC0000004) 或 STATUS_BUFFER_TOO_SMALL (0xC0000023)
...
//
// ★ 使用双层级路径解析（Trae 方案）：
//   [Tier 1] NtQueryObject — 轻量，无递归风险，<1μs
//   [Tier 2] GetFinalPathNameByHandleW — 回退，有 TLS 守卫
//
    ...
    PULONG Key)
{
    ...
    // ★ Tier 1: NtQueryObject 快速路径（无递归风险）
    wchar_t path[1024] = {0};
